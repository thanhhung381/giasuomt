...
public class JwtUltils {
	...
	public String generateJwtToken(Authentication authenticateAction) {
		...
		return Jwts.builder().setSubject(userDetails.getUsername()).setIssuedAt(date)
				.setExpiration(new Date(date.getTime() + jwtDuration)).signWith(SignatureAlgorithm.HS256, secretKey)
	...
	public String getUsernameToken(String token) {
		...
		// vì trong JWT có phần body mà trong phần body đó nó có chứa subject mà subject
		// chứa username so JWT ở trên mình đã cấu hình hihi
	}
