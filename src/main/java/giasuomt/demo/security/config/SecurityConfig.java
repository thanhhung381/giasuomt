...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Override // Mình Override lại thằng configure(HttpSecurity http) (có sẵn trong Spring
				// Security) -> để mình config việc nó security trên các cái Http Request của
				// mình.
	protected void configure(HttpSecurity http) throws Exception {
		// Enable CORS - xem lại tài liệu để hiểu CORS là gì - nói ngắn gọn là ở đây
		// mình cấu hình để cho phép nguồn nào (domain nào) có thể gửi HttpRequest tới
		// Server của mình được, và có thể gởi Request tới API nào được, API nào không.
...
//			.antMatchers("/api/**").permitAll() //permitAll (cho phép) tất cả các truy cập vô tất cả các API
				...
				.permitAll().anyRequest().authenticated(); // anthenticated (bảo mật) tất cả các request còn lại - để
															// tránh việc kẻ xấu vô khám phá xem mình còn những trang gì
															// nữa ko...
