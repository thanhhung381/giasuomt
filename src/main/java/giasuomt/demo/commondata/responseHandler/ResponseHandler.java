...
public class ResponseHandler {
	...
	public static ResponseEntity<Object> getResponse(BindingResult bindingResult, HttpStatus status){ //Khi kiểm tra Validation @Valid, có những messages lỗi gì, nó sẽ đẩy về biến BindingResult bindingResult
		Map<String, Object> map = new HashMap<>(); //Map là dạng dữ liệu t.tự như List, nhưng nó có thêm Key. Map<String, Object> là dạng List có key là dạng String và giá trị là dạng Object 
		map.put("content", ListUtils.emptyStringList); //TH này có đưa vào bindingResult nên là có lỗi, nên content là List rỗng
		map.put("message", ErrorUtils.getErrorMessages(bindingResult));  //ErrorUtils.getErrorMessages(errors) là để chỉ lấy đoạn messages - tức là defaultMessages trong ObjectError mặc định trong bindingResult.getAllErrors() [xem trong ErrorUtils] 
		map.put("status", status.value()); //status.value() là trả về status dạng số (thường trả về số luôn để thuận tiện cho FrontEnd)
	...
	public static ResponseEntity<Object> getResponse(String error, HttpStatus status){ 
		Map<String, Object> map = new HashMap<>(); //Map là dạng dữ liệu t.tự như List, nhưng nó có thêm Key. Map<String, Object> là dạng List có key là dạng String và giá trị là dạng Object 
		map.put("content", ListUtils.emptyStringList); //TH này có đưa vào errors nên là có lỗi, nên content là List rỗng
		map.put("message", ErrorUtils.errorOf(error));  //errorOf là parse từ String Message sang dạng List -> để có thể đưa vô ResponseObject để trả về [xem trong ErrorUtils] 
		map.put("status", status.value()); //status.value() là trả về status dạng số (thường trả về số luôn để thuận tiện cho FrontEnd)
